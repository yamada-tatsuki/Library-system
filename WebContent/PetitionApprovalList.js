		...
		success : function(json) {
			...
			for (var i = 0; i < json.length; i++) {
				...
				var record = '<tr>'
					+ '<td>' + element.title + '</td>'
						+ '<td>' + element.name+ '</td>'
					+ '<td><button class="no" id="delet'+ element.title+ '" title="'+ element.title +'" result="rejected">' + "却下" + '</button></td>'
					+ '<td><button class="ok" id="'+ element.title + '" result="approv" title="'+ element.title +'">' + "承認" + '</button></td>'
						+ '</tr>';
